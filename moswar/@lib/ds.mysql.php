...
class SqlDataSource implements IDataSource
{
    ...
    {
        global $configDb;
        if ($configDb['persistent']) {
            $this->serverLink = mysql_pconnect($configDb['host'], $configDb['user'], $configDb['pwd'], 65536);
        } else {
    ...
    {
        global $configDb;
        return mysql_select_db($configDb['db'], $this->serverLink);
    }
    ...
    public function query($query) {
		...
		if (self::$debug && mysql_errno()) {
			...
			$string = date("Y-m-d H:i:s") . " " . preg_replace("/[\s]{2,}/", " ", trim($query)) . " - " . mysql_errno() . ' - ' . mysql_error() . PHP_EOL;
			if (function_exists('xdebug_get_function_stack')) {
				$q = xdebug_get_function_stack();
			} else {
				$q = debug_backtrace(false);
			}
			foreach ($q as $key => &$v) {
				unset($v['args']);
			}
			$string .= print_r($q, true) . PHP_EOL;
			fwrite($fp, $string);
			fclose($fp);
		}/* else if (self::$debug) {
