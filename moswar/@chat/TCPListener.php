...
class TCPListener
{
    ...
    public function onReadyToReceive()
    {
        ...
        {
			socket_set_option($thenewsock, SOL_SOCKET, SO_RCVTIMEO, array("sec" => 1, "usec" => 0));
			socket_set_option($thenewsock, SOL_SOCKET, SO_SNDTIMEO, array("sec" => 1, "usec" => 0));
            print date("d-m-y H:i:s ") . "New Client: $thenewsock\n";
            $newClient = new ClientSock($thenewsock);
            SocketManager::addSocket($newClient, $newClient->id);
        }
    }
