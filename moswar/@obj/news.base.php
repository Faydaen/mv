<?php
class newsBaseObject extends Object
{
	public static $METAOBJECT = 'news';
    public static $ID_METAATTRIBUTE = 'news.id';
 	public static $ID = 'news.id';
    public $dt = '0000-00-00 00:00:00';
    public static $DT = 'news.dt';
    public $text = '';
    public static $TEXT = 'news.text';
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	$this->sql->query("UPDATE `news".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `news".$saveMerge."` SET `dt`='".Std::cleanString($this->dt)."', `text`='".Std::cleanString($this->text)."' WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `news".$saveMerge."` (`dt`, `text`) VALUES ('".Std::cleanString($this->dt)."', '".Std::cleanString($this->text)."')");
        	//
