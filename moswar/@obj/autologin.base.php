<?php
class autologinBaseObject extends Object
{
	public static $METAOBJECT = 'autologin';
    public static $ID_METAATTRIBUTE = 'autologin.id';
 	public static $ID = 'autologin.id';
    public $player = 0;
    public static $PLAYER = 'autologin.player';
    public $session = '';
    public static $SESSION = 'autologin.session';
    public $dt = '0000-00-00 00:00:00';
    public static $DT = 'autologin.dt';
    public $type = '';
    public static $TYPE = 'autologin.type';
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('autologin.', '', $field);
            	...
            	$this->sql->query("UPDATE `autologin".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `autologin".$saveMerge."` SET `player`=".(is_object($this->player) ? $this->player->id : $this->player).", `session`='".Std::cleanString($this->session)."', `dt`='".Std::cleanString($this->dt)."', `type`='".Std::cleanString($this->type)."' WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `autologin".$saveMerge."` (`player`, `session`, `dt`, `type`) VALUES (".(is_object($this->player) ? $this->player->id : $this->player).", '".Std::cleanString($this->session)."', '".Std::cleanString($this->dt)."', '".Std::cleanString($this->type)."')");
        	//
