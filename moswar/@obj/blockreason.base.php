<?php
class blockreasonBaseObject extends Object
{
	public static $METAOBJECT = 'blockreason';
    public static $ID_METAATTRIBUTE = 'blockreason.id';
 	public static $ID = 'blockreason.id';
    public $text = '';
    public static $TEXT = 'blockreason.text';
    public $honey = 0;
    public static $HONEY = 'blockreason.honey';

    public function __construct()
    {
        parent::__construct('blockreason');
    }
    ...
    public function initFromForm($id=0)
    {
        ...
        {
            case 159:
                $this->text = str_replace('"', '&quot;', $_POST['text']);
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('blockreason.', '', $field);
            	...
            	$this->sql->query("UPDATE `blockreason".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `blockreason".$saveMerge."` SET `text`='".Std::cleanString($this->text)."', `honey`=".(int)$this->honey." WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `blockreason".$saveMerge."` (`text`, `honey`) VALUES ('".Std::cleanString($this->text)."', ".(int)$this->honey.")");
        	//
