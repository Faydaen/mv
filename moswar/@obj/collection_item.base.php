<?php
class collection_itemBaseObject extends Object
{
	public static $METAOBJECT = 'collection_item';
    public static $ID_METAATTRIBUTE = 'collection_item.id';
 	public static $ID = 'collection_item.id';
    public $name = '';
    public static $NAME = 'collection_item.name';
    public $image = '';
    public static $IMAGE = 'collection_item.image';
    public $collection = 0;
    public static $COLLECTION = 'collection_item.collection';
    public $amount = 0;
    public static $AMOUNT = 'collection_item.amount';

    public function __construct()
    {
        parent::__construct('collection_item');
    }
    ...
    public function initFromForm($id=0)
    {
        ...
        switch ($this->metaViewId)
        {
            case 129:
                ...
                $this->collection = (int) $_POST['collection'];
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('collection_item.', '', $field);
            	...
            	$this->sql->query("UPDATE `collection_item".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `collection_item".$saveMerge."` SET `name`='".Std::cleanString($this->name)."', `image`='".Std::cleanString($this->image)."', `collection`=".(is_object($this->collection) ? $this->collection->id : $this->collection).", `amount`=".(int)$this->amount." WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `collection_item".$saveMerge."` (`name`, `image`, `collection`, `amount`) VALUES ('".Std::cleanString($this->name)."', '".Std::cleanString($this->image)."', ".(is_object($this->collection) ? $this->collection->id : $this->collection).", ".(int)$this->amount.")");
        	//
    ...
    private function getType($fieldCode)
    {
    	switch ($fieldCode) {
			...
			case 'collection': return 13; break;
