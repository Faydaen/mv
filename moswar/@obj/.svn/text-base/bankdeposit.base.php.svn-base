<?php
class bankdepositBaseObject extends Object
{
	public static $METAOBJECT = 'bankdeposit';
    public static $ID_METAATTRIBUTE = 'bankdeposit.player';
    public $player = 0;
    public static $PLAYER = 'bankdeposit.player';
    public $dt = '0000-00-00 00:00:00';
    public static $DT = 'bankdeposit.dt';
    public $enddt = '0000-00-00 00:00:00';
    public static $ENDDT = 'bankdeposit.enddt';
    public $money = 0;
    public static $MONEY = 'bankdeposit.money';

    public function __construct()
    {
        parent::__construct('bankdeposit');
    }
    ...
    public function reset()
    {
        ...
        $this->enddt = '0000-00-00 00:00:00';
        $this->money = 0;
    }
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('bankdeposit.', '', $field);
            	...
            	$this->sql->query("UPDATE `bankdeposit".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `bankdeposit".$saveMerge."` SET `player`=".(is_object($this->player) ? $this->player->id : $this->player).", `dt`='".Std::cleanString($this->dt)."', `enddt`='".Std::cleanString($this->enddt)."', `money`=".(int)$this->money." WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `bankdeposit".$saveMerge."` (`player`, `dt`, `enddt`, `money`) VALUES (".(is_object($this->player) ? $this->player->id : $this->player).", '".Std::cleanString($this->dt)."', '".Std::cleanString($this->enddt)."', ".(int)$this->money.")");
        	//
