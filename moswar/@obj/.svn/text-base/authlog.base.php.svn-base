<?php
class authlogBaseObject extends Object
{
	public static $METAOBJECT = 'authlog';
    public static $ID_METAATTRIBUTE = 'authlog.id';
 	public static $ID = 'authlog.id';
    public $player = 0;
    public static $PLAYER = 'authlog.player';
    public $ip = '';
    public static $IP = 'authlog.ip';
    public $browser = '';
    public static $BROWSER = 'authlog.browser';
    public $time = '0000-00-00 00:00:00';
    public static $TIME = 'authlog.time';
    public $info = '';
    public static $INFO = 'authlog.info';

    public function __construct()
    {
        parent::__construct('authlog');
    }
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('authlog.', '', $field);
            	...
            	$this->sql->query("UPDATE `authlog".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `authlog".$saveMerge."` SET `player`=".(is_object($this->player) ? $this->player->id : $this->player).", `ip`='".Std::cleanString($this->ip)."', `browser`='".Std::cleanString($this->browser)."', `time`='".Std::cleanString($this->time)."', `info`='".Std::cleanString($this->info)."' WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `authlog".$saveMerge."` (`player`, `ip`, `browser`, `time`, `info`) VALUES (".(is_object($this->player) ? $this->player->id : $this->player).", '".Std::cleanString($this->ip)."', '".Std::cleanString($this->browser)."', '".Std::cleanString($this->time)."', '".Std::cleanString($this->info)."')");
        	//
    ...
    private function getType($fieldCode)
    {
    	switch ($fieldCode) {
			...
			case 'ip': return 4; break;
			case 'browser': return 4; break;
