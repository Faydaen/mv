<?php
class factory_investitionBaseObject extends Object
{
	public static $METAOBJECT = 'factory_investition';
    public static $ID_METAATTRIBUTE = 'factory_investition.id';
 	public static $ID = 'factory_investition.id';
    public $player = 0;
    public static $PLAYER = 'factory_investition.player';
    public $money = 0;
    public static $MONEY = 'factory_investition.money';
    public $ore = 0;
    public static $ORE = 'factory_investition.ore';
    public $honey = 0;
    public static $HONEY = 'factory_investition.honey';

    public function __construct()
    {
        parent::__construct('factory_investition');
    }
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('factory_investition.', '', $field);
            	...
            	$this->sql->query("UPDATE `factory_investition".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `factory_investition".$saveMerge."` SET `player`=".(is_object($this->player) ? $this->player->id : $this->player).", `money`=".(int)$this->money.", `ore`=".(int)$this->ore.", `honey`=".(int)$this->honey." WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `factory_investition".$saveMerge."` (`player`, `money`, `ore`, `honey`) VALUES (".(is_object($this->player) ? $this->player->id : $this->player).", ".(int)$this->money.", ".(int)$this->ore.", ".(int)$this->honey.")");
        	//
