<?php
class sovetlogBaseObject extends Object
{
	public static $METAOBJECT = 'sovetlog';
    public static $ID_METAATTRIBUTE = 'sovetlog.id';
 	public static $ID = 'sovetlog.id';
    public $player = 0;
    public static $PLAYER = 'sovetlog.player';
    public $fraction = '';
    public $fraction_Dictionary = array('arrived','resident');
    public static $FRACTION = 'sovetlog.fraction';
    public $dt = '0000-00-00 00:00:00';
    public static $DT = 'sovetlog.dt';
    public $action = '';
    public static $ACTION = 'sovetlog.action';
    public $params = '';
    public static $PARAMS = 'sovetlog.params';

    public function __construct()
    {
        parent::__construct('sovetlog');
    }
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('sovetlog.', '', $field);
            	...
            	$this->sql->query("UPDATE `sovetlog".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `sovetlog".$saveMerge."` SET `player`=".(is_object($this->player) ? $this->player->id : $this->player).", `fraction`='".Std::cleanString($this->fraction)."', `dt`='".Std::cleanString($this->dt)."', `action`='".Std::cleanString($this->action)."', `params`='".Std::cleanString($this->params)."' WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `sovetlog".$saveMerge."` (`player`, `fraction`, `dt`, `action`, `params`) VALUES (".(is_object($this->player) ? $this->player->id : $this->player).", '".Std::cleanString($this->fraction)."', '".Std::cleanString($this->dt)."', '".Std::cleanString($this->action)."', '".Std::cleanString($this->params)."')");
        	//
