<?php
class accessBaseObject extends Object
{
	public static $METAOBJECT = 'access';
    public static $ID_METAATTRIBUTE = 'access.id';
 	public static $ID = 'access.id';
    public $code = '';
    public static $CODE = 'access.code';
    public $title = '';
    public static $TITLE = 'access.title';
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('access.', '', $field);
            	...
            	$this->sql->query("UPDATE `access".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `access".$saveMerge."` SET `code`='".Std::cleanString($this->code)."', `title`='".Std::cleanString($this->title)."' WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `access".$saveMerge."` (`code`, `title`) VALUES ('".Std::cleanString($this->code)."', '".Std::cleanString($this->title)."')");
        	//
