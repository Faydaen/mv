<?php
class rating_clanBaseObject extends Object
{
	public static $METAOBJECT = 'rating_clan';
    public static $ID_METAATTRIBUTE = 'rating_clan.clan';
    public $clan = 0;
    public static $CLAN = 'rating_clan.clan';
    public $exp = 0;
    public static $EXP = 'rating_clan.exp';
    public $fraction = '';
    public $fraction_Dictionary = array('arrived','resident');
    public static $FRACTION = 'rating_clan.fraction';
    public $money = 0;
    public static $MONEY = 'rating_clan.money';
    public $honey = 0;
    public static $HONEY = 'rating_clan.honey';
    public $ore = 0;
    public static $ORE = 'rating_clan.ore';

    public function __construct()
    {
        parent::__construct('rating_clan');
    }
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('rating_clan.', '', $field);
            	...
            	$this->sql->query("UPDATE `rating_clan".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `rating_clan".$saveMerge."` SET `clan`=".(int)$this->clan.", `exp`=".(int)$this->exp.", `fraction`='".Std::cleanString($this->fraction)."', `money`=".(int)$this->money.", `honey`=".(int)$this->honey.", `ore`=".(int)$this->ore." WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `rating_clan".$saveMerge."` (`clan`, `exp`, `fraction`, `money`, `honey`, `ore`) VALUES (".(int)$this->clan.", ".(int)$this->exp.", '".Std::cleanString($this->fraction)."', ".(int)$this->money.", ".(int)$this->honey.", ".(int)$this->ore.")");
        	//
