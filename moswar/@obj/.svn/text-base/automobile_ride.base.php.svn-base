<?php
class automobile_rideBaseObject extends Object
{
	public static $METAOBJECT = 'automobile_ride';
    public static $ID_METAATTRIBUTE = 'automobile_ride.id';
 	public static $ID = 'automobile_ride.id';
    public $player = 0;
    public static $PLAYER = 'automobile_ride.player';
    public $car = 0;
    public static $CAR = 'automobile_ride.car';
    public $direction = 0;
    public static $DIRECTION = 'automobile_ride.direction';
    public $fail = 0;
    public static $FAIL = 'automobile_ride.fail';
    public $cooldown = 0;
    public static $COOLDOWN = 'automobile_ride.cooldown';

    public function __construct()
    {
        parent::__construct('automobile_ride');
    }
    ...
    public function initFromForm($id=0)
    {
        ...
        switch ($this->metaViewId)
        {
            case 186:
                ...
                $this->car = (int) $_POST['car'];
                $this->direction = (int) $_POST['direction'];
                $this->fail = (int) $_POST['fail'];
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('automobile_ride.', '', $field);
            	...
            	$this->sql->query("UPDATE `automobile_ride".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `automobile_ride".$saveMerge."` SET `player`=".(is_object($this->player) ? $this->player->id : $this->player).", `car`=".(is_object($this->car) ? $this->car->id : $this->car).", `direction`=".(int)$this->direction.", `fail`=".(int)$this->fail.", `cooldown`=".(int)$this->cooldown." WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `automobile_ride".$saveMerge."` (`player`, `car`, `direction`, `fail`, `cooldown`) VALUES (".(is_object($this->player) ? $this->player->id : $this->player).", ".(is_object($this->car) ? $this->car->id : $this->car).", ".(int)$this->direction.", ".(int)$this->fail.", ".(int)$this->cooldown.")");
        	//
    ...
    private function getType($fieldCode)
    {
    	switch ($fieldCode) {
			...
			case 'car': return 13; break;
			case 'direction': return 2; break;
			case 'fail': return 2; break;
