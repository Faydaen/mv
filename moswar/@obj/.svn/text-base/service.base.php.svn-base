<?php
class serviceBaseObject extends Object
{
	public static $METAOBJECT = 'service';
    public static $ID_METAATTRIBUTE = 'service.id';
 	public static $ID = 'service.id';
    public $code = '';
    public static $CODE = 'service.code';
    public $title = '';
    public static $TITLE = 'service.title';
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('service.', '', $field);
            	...
            	$this->sql->query("UPDATE `service".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `service".$saveMerge."` SET `code`='".Std::cleanString($this->code)."', `title`='".Std::cleanString($this->title)."' WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `service".$saveMerge."` (`code`, `title`) VALUES ('".Std::cleanString($this->code)."', '".Std::cleanString($this->title)."')");
        	//
