<?php
class contactBaseObject extends Object
{
	public static $METAOBJECT = 'contact';
    public static $ID_METAATTRIBUTE = 'contact.id';
 	public static $ID = 'contact.id';
    public $player = 0;
    public static $PLAYER = 'contact.player';
    public $player2 = 0;
    public static $PLAYER2 = 'contact.player2';
    public $type = '';
    public $type_Dictionary = array('friend','enemy','victim','black','referer');
    public static $TYPE = 'contact.type';
    public $info = '';
    public static $INFO = 'contact.info';
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('contact.', '', $field);
            	...
            	$this->sql->query("UPDATE `contact".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `contact".$saveMerge."` SET `player`=".(is_object($this->player) ? $this->player->id : $this->player).", `player2`=".(is_object($this->player2) ? $this->player2->id : $this->player2).", `type`='".Std::cleanString($this->type)."', `info`='".Std::cleanString($this->info)."' WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `contact".$saveMerge."` (`player`, `player2`, `type`, `info`) VALUES (".(is_object($this->player) ? $this->player->id : $this->player).", ".(is_object($this->player2) ? $this->player2->id : $this->player2).", '".Std::cleanString($this->type)."', '".Std::cleanString($this->info)."')");
        	//
