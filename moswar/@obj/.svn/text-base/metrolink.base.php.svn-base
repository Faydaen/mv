<?php
class metrolinkBaseObject extends Object
{
	public static $METAOBJECT = 'metrolink';
    public static $ID_METAATTRIBUTE = 'metrolink.id';
 	public static $ID = 'metrolink.id';
    public $metro1 = 0;
    public static $METRO1 = 'metrolink.metro1';
    public $metro2 = 0;
    public static $METRO2 = 'metrolink.metro2';

    public function __construct()
    {
        parent::__construct('metrolink');
    }
    ...
    public function init($object)
    {
        ...
        $this->metro1 = $object['metro1'];
        $this->metro2 = $object['metro2'];
    }
    ...
    public function initFromForm($id=0)
    {
        ...
        {
            case 141:
                $this->metro1 = (int) $_POST['metro1'];
                $this->metro2 = (int) $_POST['metro2'];
                break;
        }
    }
    ...
    public function toArray()
    {
        $object = array();
        $object['id'] = $this->id;
        if (is_object($this->metro1))
        {
            $object['metro1'] = $this->metro1->toArray();
        }
        else
        {
        	$object['metro1'] = $this->metro1;
        }
        if (is_object($this->metro2))
        {
            $object['metro2'] = $this->metro2->toArray();
        }
        else
        {
        	$object['metro2'] = $this->metro2;
        }
        return $object;
    }
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('metrolink.', '', $field);
            	...
            	$this->sql->query("UPDATE `metrolink".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `metrolink".$saveMerge."` SET `metro1`=".(is_object($this->metro1) ? $this->metro1->id : $this->metro1).", `metro2`=".(is_object($this->metro2) ? $this->metro2->id : $this->metro2)." WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `metrolink".$saveMerge."` (`metro1`, `metro2`) VALUES (".(is_object($this->metro1) ? $this->metro1->id : $this->metro1).", ".(is_object($this->metro2) ? $this->metro2->id : $this->metro2).")");
        	//
    ...
    private function getType($fieldCode)
    {
    	switch ($fieldCode) {
			...
			case 'metro1': return 13; break;
			case 'metro2': return 13; break;
    	}
    }
}
?>