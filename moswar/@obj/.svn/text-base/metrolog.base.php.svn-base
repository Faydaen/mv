<?php
class metrologBaseObject extends Object
{
	public static $METAOBJECT = 'metrolog';
    public static $ID_METAATTRIBUTE = 'metrolog.id';
 	public static $ID = 'metrolog.id';
    public $player = 0;
    public static $PLAYER = 'metrolog.player';
    public $dt = '0000-00-00 00:00:00';
    public static $DT = 'metrolog.dt';
    public $result = '';
    public $result_Dictionary = array('success','fail');
    public static $RESULT = 'metrolog.result';

    public function __construct()
    {
        parent::__construct('metrolog');
    }
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('metrolog.', '', $field);
            	...
            	$this->sql->query("UPDATE `metrolog".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `metrolog".$saveMerge."` SET `player`=".(is_object($this->player) ? $this->player->id : $this->player).", `dt`='".Std::cleanString($this->dt)."', `result`='".Std::cleanString($this->result)."' WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `metrolog".$saveMerge."` (`player`, `dt`, `result`) VALUES (".(is_object($this->player) ? $this->player->id : $this->player).", '".Std::cleanString($this->dt)."', '".Std::cleanString($this->result)."')");
        	//
