<?php
class clanlogBaseObject extends Object
{
	public static $METAOBJECT = 'clanlog';
    public static $ID_METAATTRIBUTE = 'clanlog.id';
 	public static $ID = 'clanlog.id';
    public $player = 0;
    public static $PLAYER = 'clanlog.player';
    public $dt = '0000-00-00 00:00:00';
    public static $DT = 'clanlog.dt';
    public $action = '';
    public static $ACTION = 'clanlog.action';
    public $params = '';
    public static $PARAMS = 'clanlog.params';
    public $clan = 0;
    public static $CLAN = 'clanlog.clan';

    public function __construct()
    {
        parent::__construct('clanlog');
    }
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('clanlog.', '', $field);
            	...
            	$this->sql->query("UPDATE `clanlog".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `clanlog".$saveMerge."` SET `player`=".(is_object($this->player) ? $this->player->id : $this->player).", `dt`='".Std::cleanString($this->dt)."', `action`='".Std::cleanString($this->action)."', `params`='".Std::cleanString($this->params)."', `clan`=".(is_object($this->clan) ? $this->clan->id : $this->clan)." WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `clanlog".$saveMerge."` (`player`, `dt`, `action`, `params`, `clan`) VALUES (".(is_object($this->player) ? $this->player->id : $this->player).", '".Std::cleanString($this->dt)."', '".Std::cleanString($this->action)."', '".Std::cleanString($this->params)."', ".(is_object($this->clan) ? $this->clan->id : $this->clan).")");
        	//
