<?php
class questionBaseObject extends Object
{
	public static $METAOBJECT = 'question';
    public static $ID_METAATTRIBUTE = 'question.id';
 	public static $ID = 'question.id';
    public $question = '';
    public static $QUESTION = 'question.question';
    public $answer = '';
    public static $ANSWER = 'question.answer';
    ...
    public function initFromForm($id=0)
    {
        ...
        switch ($this->metaViewId)
        {
            case 126:
                ...
                $this->answer = str_replace('"', '&quot;', $_POST['answer']);
                break;
        }
    }
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('question.', '', $field);
            	...
            	$this->sql->query("UPDATE `question".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `question".$saveMerge."` SET `question`='".Std::cleanString($this->question)."', `answer`='".Std::cleanString($this->answer)."' WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `question".$saveMerge."` (`question`, `answer`) VALUES ('".Std::cleanString($this->question)."', '".Std::cleanString($this->answer)."')");
        	//
