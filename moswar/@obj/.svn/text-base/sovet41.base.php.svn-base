<?php
class sovet41BaseObject extends Object
{
	public static $METAOBJECT = 'sovet41';
    public static $ID_METAATTRIBUTE = 'sovet41.id';
 	public static $ID = 'sovet41.id';
    public $fraction = '';
    public $fraction_Dictionary = array('arrived','resident');
    public static $FRACTION = 'sovet41.fraction';
    public $player = 0;
    public static $PLAYER = 'sovet41.player';
    public $status = '';
    public $status_Dictionary = array('founder','accepted');
    public static $STATUS = 'sovet41.status';
    public $year = 0;
    public static $YEAR = 'sovet41.year';
    public $week = 0;
    public static $WEEK = 'sovet41.week';
    public $num = 0;
    public static $NUM = 'sovet41.num';
    public $metro = 0;
    public static $METRO = 'sovet41.metro';
    public $result = 0;
    public static $RESULT = 'sovet41.result';

    public function __construct()
    {
        parent::__construct('sovet41');
    }
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('sovet41.', '', $field);
            	...
            	$this->sql->query("UPDATE `sovet41".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `sovet41".$saveMerge."` SET `fraction`='".Std::cleanString($this->fraction)."', `player`=".(is_object($this->player) ? $this->player->id : $this->player).", `status`='".Std::cleanString($this->status)."', `year`=".(int)$this->year.", `week`=".(int)$this->week.", `num`=".(int)$this->num.", `metro`=".(is_object($this->metro) ? $this->metro->id : $this->metro).", `result`=".(int)$this->result." WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `sovet41".$saveMerge."` (`fraction`, `player`, `status`, `year`, `week`, `num`, `metro`, `result`) VALUES ('".Std::cleanString($this->fraction)."', ".(is_object($this->player) ? $this->player->id : $this->player).", '".Std::cleanString($this->status)."', ".(int)$this->year.", ".(int)$this->week.", ".(int)$this->num.", ".(is_object($this->metro) ? $this->metro->id : $this->metro).", ".(int)$this->result.")");
        	//
