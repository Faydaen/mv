<?php
class player_topic_readBaseObject extends Object
{
	public static $METAOBJECT = 'player_topic_read';
    public $player = 0;
    public static $PLAYER = 'player_topic_read.player';
    public $topic = 0;
    public static $TOPIC = 'player_topic_read.topic';
    public $post = 0;
    public static $POST = 'player_topic_read.post';

    public function __construct()
    {
        parent::__construct('player_topic_read');
    }
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('player_topic_read.', '', $field);
            	...
            	$this->sql->query("UPDATE `player_topic_read".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `player_topic_read".$saveMerge."` SET `player`=".(is_object($this->player) ? $this->player->id : $this->player).", `topic`=".(is_object($this->topic) ? $this->topic->id : $this->topic).", `post`=".(is_object($this->post) ? $this->post->id : $this->post)." WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `player_topic_read".$saveMerge."` (`player`, `topic`, `post`) VALUES (".(is_object($this->player) ? $this->player->id : $this->player).", ".(is_object($this->topic) ? $this->topic->id : $this->topic).", ".(is_object($this->post) ? $this->post->id : $this->post).")");
        	//
