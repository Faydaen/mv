<?php
class itemdroplogBaseObject extends Object
{
	public static $METAOBJECT = 'itemdroplog';
    public static $ID_METAATTRIBUTE = 'itemdroplog.id';
 	public static $ID = 'itemdroplog.id';
    public $player = 0;
    public static $PLAYER = 'itemdroplog.player';
    public $itemdrop = 0;
    public static $ITEMDROP = 'itemdroplog.itemdrop';
    public $time = 0;
    public static $TIME = 'itemdroplog.time';

    public function __construct()
    {
        parent::__construct('itemdroplog');
    }
    ...
    public function toArray()
    {
        ...
        if (is_object($this->itemdrop))
        {
            $object['itemdrop'] = $this->itemdrop->toArray();
        }
        else
        {
        	$object['itemdrop'] = $this->itemdrop;
        }
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('itemdroplog.', '', $field);
            	...
            	$this->sql->query("UPDATE `itemdroplog".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `itemdroplog".$saveMerge."` SET `player`=".(is_object($this->player) ? $this->player->id : $this->player).", `itemdrop`=".(is_object($this->itemdrop) ? $this->itemdrop->id : $this->itemdrop).", `time`=".(int)$this->time." WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `itemdroplog".$saveMerge."` (`player`, `itemdrop`, `time`) VALUES (".(is_object($this->player) ? $this->player->id : $this->player).", ".(is_object($this->itemdrop) ? $this->itemdrop->id : $this->itemdrop).", ".(int)$this->time.")");
        	//
