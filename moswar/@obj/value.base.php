<?php
class valueBaseObject extends Object
{
	public static $METAOBJECT = 'value';
    public static $ID_METAATTRIBUTE = 'value.name';
    public $name = '';
    public static $NAME = 'value.name';
    public $value = '';
    public static $VALUE = 'value.value';
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	$this->sql->query("UPDATE `value".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `value".$saveMerge."` SET `name`='".Std::cleanString($this->name)."', `value`='".Std::cleanString($this->value)."' WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `value".$saveMerge."` (`name`, `value`) VALUES ('".Std::cleanString($this->name)."', '".Std::cleanString($this->value)."')");
        	//
