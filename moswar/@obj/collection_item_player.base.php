<?php
class collection_item_playerBaseObject extends Object
{
	public static $METAOBJECT = 'collection_item_player';
    public $player = 0;
    public static $PLAYER = 'collection_item_player.player';
    public $collection_item = 0;
    public static $COLLECTION_ITEM = 'collection_item_player.collection_item';
    public $collection = 0;
    public static $COLLECTION = 'collection_item_player.collection';
    public $amount = 0;
    public static $AMOUNT = 'collection_item_player.amount';

    public function __construct()
    {
        parent::__construct('collection_item_player');
    }
    ...
    public function init($object)
    {
        ...
        $this->collection_item = $object['collection_item'];
    ...
    public function toArray()
    {
        ...
        if (is_object($this->collection_item))
        {
            $object['collection_item'] = $this->collection_item->toArray();
        }
        else
        {
        	$object['collection_item'] = $this->collection_item;
        }
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('collection_item_player.', '', $field);
            	...
            	$this->sql->query("UPDATE `collection_item_player".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `collection_item_player".$saveMerge."` SET `player`=".(is_object($this->player) ? $this->player->id : $this->player).", `collection_item`=".(is_object($this->collection_item) ? $this->collection_item->id : $this->collection_item).", `collection`=".(is_object($this->collection) ? $this->collection->id : $this->collection).", `amount`=".(int)$this->amount." WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `collection_item_player".$saveMerge."` (`player`, `collection_item`, `collection`, `amount`) VALUES (".(is_object($this->player) ? $this->player->id : $this->player).", ".(is_object($this->collection_item) ? $this->collection_item->id : $this->collection_item).", ".(is_object($this->collection) ? $this->collection->id : $this->collection).", ".(int)$this->amount.")");
        	//
    ...
    private function getType($fieldCode)
    {
    	switch ($fieldCode) {
			...
			case 'collection_item': return 13; break;
