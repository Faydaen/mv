<?php
class sysconfigBaseObject extends Object
{
	public static $METAOBJECT = 'sysconfig';
    public static $ID_METAATTRIBUTE = 'sysconfig.id';
 	public static $ID = 'sysconfig.id';
    public $code = '';
    public static $CODE = 'sysconfig.code';
    public $value = '';
    public static $VALUE = 'sysconfig.value';
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('sysconfig.', '', $field);
            	...
            	$this->sql->query("UPDATE `sysconfig".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `sysconfig".$saveMerge."` SET `code`='".Std::cleanString($this->code)."', `value`='".Std::cleanString($this->value)."' WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `sysconfig".$saveMerge."` (`code`, `value`) VALUES ('".Std::cleanString($this->code)."', '".Std::cleanString($this->value)."')");
        	//
