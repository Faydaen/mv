<?php
class playerworkBaseObject extends Object
{
	public static $METAOBJECT = 'playerwork';
    public static $ID_METAATTRIBUTE = 'playerwork.player';
    public $player = 0;
    public static $PLAYER = 'playerwork.player';
    public $endtime = 0;
    public static $ENDTIME = 'playerwork.endtime';
    public $salary = 0;
    public static $SALARY = 'playerwork.salary';
    public $exp = 0;
    public static $EXP = 'playerwork.exp';
    public $type = '';
    public $type_Dictionary = array('patrol','macdonalds','metro','petriks','training');
    public static $TYPE = 'playerwork.type';
    public $time = 0;
    public static $TIME = 'playerwork.time';
    public $params = '';
    public static $PARAMS = 'playerwork.params';

    public function __construct()
    {
        parent::__construct('playerwork');
    }
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('playerwork.', '', $field);
            	...
            	$this->sql->query("UPDATE `playerwork".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `playerwork".$saveMerge."` SET `player`=".(is_object($this->player) ? $this->player->id : $this->player).", `endtime`=".(int)$this->endtime.", `salary`=".(int)$this->salary.", `exp`=".(int)$this->exp.", `type`='".Std::cleanString($this->type)."', `time`=".(int)$this->time.", `params`='".Std::cleanString($this->params)."' WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `playerwork".$saveMerge."` (`player`, `endtime`, `salary`, `exp`, `type`, `time`, `params`) VALUES (".(is_object($this->player) ? $this->player->id : $this->player).", ".(int)$this->endtime.", ".(int)$this->salary.", ".(int)$this->exp.", '".Std::cleanString($this->type)."', ".(int)$this->time.", '".Std::cleanString($this->params)."')");
        	//
