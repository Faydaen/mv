<?php
class mapBaseObject extends Object
{
	public static $METAOBJECT = 'map';
    public static $ID_METAATTRIBUTE = 'map.id';
 	public static $ID = 'map.id';
    public $code = '';
    public static $CODE = 'map.code';
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	$this->sql->query("UPDATE `map".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `map".$saveMerge."` SET `code`='".Std::cleanString($this->code)."' WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `map".$saveMerge."` (`code`) VALUES ('".Std::cleanString($this->code)."')");
        	//
