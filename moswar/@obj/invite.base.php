<?php
class inviteBaseObject extends Object
{
	public static $METAOBJECT = 'invite';
    public static $ID_METAATTRIBUTE = 'invite.id';
 	public static $ID = 'invite.id';
    public $invite = '';
    public static $INVITE = 'invite.invite';
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('invite.', '', $field);
            	...
            	$this->sql->query("UPDATE `invite".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `invite".$saveMerge."` SET `invite`='".Std::cleanString($this->invite)."' WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `invite".$saveMerge."` (`invite`) VALUES ('".Std::cleanString($this->invite)."')");
        	//
