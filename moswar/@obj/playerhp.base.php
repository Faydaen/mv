<?php
class playerhpBaseObject extends Object
{
	public static $METAOBJECT = 'playerhp';
    public static $ID_METAATTRIBUTE = 'playerhp.player';
    public $player = 0;
    public static $PLAYER = 'playerhp.player';
    public $hp = 0;
    public static $HP = 'playerhp.hp';
    public $maxhp = 0;
    public static $MAXHP = 'playerhp.maxhp';
    public $regen = 0;
    public static $REGEN = 'playerhp.regen';

    public function __construct()
    {
        parent::__construct('playerhp');
    }
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('playerhp.', '', $field);
            	...
            	$this->sql->query("UPDATE `playerhp".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `playerhp".$saveMerge."` SET `player`=".(is_object($this->player) ? $this->player->id : $this->player).", `hp`=".(int)$this->hp.", `maxhp`=".(int)$this->maxhp.", `regen`=".(int)$this->regen." WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `playerhp".$saveMerge."` (`player`, `hp`, `maxhp`, `regen`) VALUES (".(is_object($this->player) ? $this->player->id : $this->player).", ".(int)$this->hp.", ".(int)$this->maxhp.", ".(int)$this->regen.")");
        	//
