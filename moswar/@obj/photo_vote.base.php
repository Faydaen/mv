<?php
class photo_voteBaseObject extends Object
{
	public static $METAOBJECT = 'photo_vote';
    public $photo = 0;
    public static $PHOTO = 'photo_vote.photo';
    public $player = 0;
    public static $PLAYER = 'photo_vote.player';
    public static $ID_METAATTRIBUTE = 'photo_vote.id';
 	public static $ID = 'photo_vote.id';
    public $dt = '0000-00-00';
    public static $DT = 'photo_vote.dt';
    public $value = 0;
    public static $VALUE = 'photo_vote.value';
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('photo_vote.', '', $field);
            	...
            	$this->sql->query("UPDATE `photo_vote".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `photo_vote".$saveMerge."` SET `photo`=".(is_object($this->photo) ? $this->photo->id : $this->photo).", `player`=".(is_object($this->player) ? $this->player->id : $this->player).", `dt`='".Std::cleanString($this->dt)."', `value`=".(int)$this->value." WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `photo_vote".$saveMerge."` (`photo`, `player`, `dt`, `value`) VALUES (".(is_object($this->photo) ? $this->photo->id : $this->photo).", ".(is_object($this->player) ? $this->player->id : $this->player).", '".Std::cleanString($this->dt)."', ".(int)$this->value.")");
        	//
