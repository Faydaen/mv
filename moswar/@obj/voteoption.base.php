<?php
class voteoptionBaseObject extends Object
{
	public static $METAOBJECT = 'voteoption';
    public static $ID_METAATTRIBUTE = 'voteoption.id';
 	public static $ID = 'voteoption.id';
    public $topic = 0;
    public static $TOPIC = 'voteoption.topic';
    public $option = '';
    public static $OPTION = 'voteoption.option';

    public function __construct()
    {
        parent::__construct('voteoption');
    }
    ...
    public function initFromForm($id=0)
    {
        ...
        switch ($this->metaViewId)
        {
            case 37:
                ...
                $this->option = str_replace('"', '&quot;', $_POST['option']);
                break;
        }
    }
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('voteoption.', '', $field);
            	...
            	$this->sql->query("UPDATE `voteoption".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `voteoption".$saveMerge."` SET `topic`=".(is_object($this->topic) ? $this->topic->id : $this->topic).", `option`='".Std::cleanString($this->option)."' WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `voteoption".$saveMerge."` (`topic`, `option`) VALUES (".(is_object($this->topic) ? $this->topic->id : $this->topic).", '".Std::cleanString($this->option)."')");
        	//
