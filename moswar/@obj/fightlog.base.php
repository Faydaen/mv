<?php
class fightlogBaseObject extends Object
{
	public static $METAOBJECT = 'fightlog';
    public static $ID_METAATTRIBUTE = 'fightlog.id';
 	public static $ID = 'fightlog.id';
    public $fight = 0;
    public static $FIGHT = 'fightlog.fight';
    public $step = 0;
    public static $STEP = 'fightlog.step';
    public $log = '';
    public static $LOG = 'fightlog.log';

    public function __construct()
    {
        parent::__construct('fightlog');
    }
    ...
    public function save($id=0, $fields=false, $saveMerge='')
    {
        ...
        if ($this->id)
        {
            ...
            if ($fields) {
            	...
            	foreach ($fields as $field) {
            		$field = str_replace('fightlog.', '', $field);
            	...
            	$this->sql->query("UPDATE `fightlog".$saveMerge."` SET " . implode(', ', $attributes) . " WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
            } else {
        		$this->sql->query("UPDATE `fightlog".$saveMerge."` SET `fight`=".(is_object($this->fight) ? $this->fight->id : $this->fight).", `step`=".(int)$this->step.", `log`='".Std::cleanString($this->log)."' WHERE ".str_replace('.', $saveMerge . '.', self::$ID_METAATTRIBUTE)."=".$this->id);
        	}
        ...
        else
        {
            ...
            //
            $this->id = $this->sql->insert("INSERT INTO `fightlog".$saveMerge."` (`fight`, `step`, `log`) VALUES (".(is_object($this->fight) ? $this->fight->id : $this->fight).", ".(int)$this->step.", '".Std::cleanString($this->log)."')");
        	//
